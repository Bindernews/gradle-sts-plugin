...
class ModMetaExtension(
    ...
    /**
     * Returns a [FileCollection] referencing the base game JAR file.
     *
     * ## Example
     * ```kt
     * dependencies {
     ...
     * the value from `steam.dir`.
     *
     * Note: You may define these values in your `gradle.properties`.
     *
     * @throws [RuntimeException] if neither property was set, and [stsHome] is not set at the end of the function
     */
    fun searchFromProperties() {
        val steamDir = project.findProperty("steam.dir")?.let { File(it as String) }
        val stsHomeFile = project.findProperty("sts.home")?.let { File(it as String) }
        if (steamDir != null && steamDir.isDirectory) {
            useSteam(steamDir)
        }
        if (stsHomeFile != null && stsHomeFile.isDirectory) {
            stsHome.set(stsHomeFile)
        }
        if (!stsHome.isPresent) {
            throw RuntimeException("StS home was not set from properties")
        }
    }
